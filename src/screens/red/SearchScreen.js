  ...
  return (
    <View className="app-container">
      ...
      <View className="flex items-center px-4 py-3">
        <TouchableOpacity onClick={() => navigate('/home')} className="text-white mr-4">
      ...
      <View className="flex-1 overflow-y-auto pb-24 px-4">
        <Text className="block text-white font-semibold text-lg mb-4">Suggestions</Text>
        ...
        <View className="space-y-4">
          {filteredSuggestions.map((suggestion) => (
            <View key={suggestion.id} className="flex items-center gap-3">
              ...
              <View>
                <Text className="block text-white font-semibold text-base">{suggestion.name}</Text>
                <Text className="block text-white/60 text-sm">{suggestion.username}</Text>
              </View>
            </View>
          ))}
          {filteredSuggestions.length === 0 && (
            <Text className="block text-white/60 text-sm">No results found</Text>
          )}
        </View>
      </View>
