      ...
      <View className="flex items-center justify-between px-4 py-3">
        ...
        <View className="flex items-center gap-4">
          ...
          <TouchableOpacity onPress={() => navigate('/notifications')}>
            <Image source={{ uri: notificationIcon }} accessibilityLabel="Notifications" className="w-6 h-6" />
          </TouchableOpacity>
        </View>
      </View>
        ...
        {postsData.map((post) => (
          <View key={post.id} className="mb-0">
              ...
              <View className="flex items-center gap-3">
                <View className="w-10 h-10 rounded-full bg-white overflow-hidden flex items-center justify-center">
                  {post.username === 'wanderlust_anna' && (
                    <Image source={{ uri: annaAvatar }} accessibilityLabel={post.username} className="w-full h-full" resizeMode="cover" />
                  )}
                  {post.username === 'EcoWear' && (
                    <Image source={{ uri: ecowearAvatar }} accessibilityLabel={post.username} className="w-full h-full" resizeMode="cover" />
                  )}
                  {post.username === 'foodie_dave' && (
                    <Image source={{ uri: daveAvatar }} accessibilityLabel={post.username} className="w-full h-full" resizeMode="cover" />
                  )}
                </View>
                <View>
            ...
            <View className="w-full aspect-square bg-black/20">
              {post.username === 'wanderlust_anna' && (
                <Image source={{ uri: annaPost }} accessibilityLabel="Post" className="w-full h-full" resizeMode="cover" />
              )}
              {post.username === 'EcoWear' && (
                <Image source={{ uri: ecowearPost }} accessibilityLabel="Post" className="w-full h-full" resizeMode="cover" />
              )}
              {post.username === 'foodie_dave' && (
                <Image source={{ uri: davePost }} accessibilityLabel="Post" className="w-full h-full" resizeMode="cover" />
              )}
            </View>
            ...
            <View className="px-4 pb-4">
              ...
              {post.learnMore && (
                <Text className="text-red-400 text-sm font-medium mt-1">Learn More</Text>
              )}
              {post.comments > 0 && (
